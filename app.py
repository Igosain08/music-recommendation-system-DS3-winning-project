...
def init_recommender():
    ...
    try:
        ...
        max_init_time = 30  # 30 seconds max
        ...
        # Initialize recommender with path to your dataset
        data_path = './music_data_with_clusters.csv'  # Use relative path in container
        ...
        # Create recommender with timeout
        try:
            recommender = EnhancedMusicRecommender(data_path)
            print("Created recommender successfully")
        except Exception as e:
            print(f"Error creating recommender: {e}")
            # Create a fallback recommender
            recommender = EnhancedMusicRecommender(None)  # Use sample data
        ...
        if time.time() - start_time > max_init_time:
            raise Exception("Initialization timeout exceeded")
        
        # Minimal delay to ensure all components are ready
        time.sleep(0.1)
...
def create_spotify_playlist():
    ...
    # Extract track IDs and validate them
    track_ids = []
    valid_track_ids = []
    for song in top_songs:
        track_id = song.get('track_id') or song.get('song_id')
        if track_id and len(track_id) == 22:  # Spotify track IDs are 22 characters
            track_ids.append(track_id)
            valid_track_ids.append(track_id)
    
    if not valid_track_ids:
        flash('No valid Spotify track IDs found in your rated songs.', 'warning')
        return redirect(url_for('history'))
    
    # Limit to 50 tracks for Spotify
    valid_track_ids = valid_track_ids[:50]
    
    # Create Spotify playlist URL with multiple tracks
    if len(valid_track_ids) == 1:
        spotify_url = f"https://open.spotify.com/track/{valid_track_ids[0]}"
    else:
        # For multiple tracks, create a comma-separated list
        track_list = ','.join(valid_track_ids)
        spotify_url = f"https://open.spotify.com/track/{valid_track_ids[0]}"
...
def health():
    ...
    # Always return 200 for health check to pass
    return "OK", 200
...
try:
    ...
    recommender = EnhancedMusicRecommender(None)
